...
@router.get("/state")
async def get_state(guild_id: int, user=Depends(guild_member), bot=Depends(get_bot)):
...
@router.get("/history")
async def get_history(guild_id: int, user=Depends(guild_member), bot=Depends(get_bot)):
...
async def clear_history(guild_id: int, request: Request, user=Depends(guild_member), bot=Depends(get_bot)):
    ...
    await broadcast_state(bot, request.app.state.ws_manager, guild_id)
